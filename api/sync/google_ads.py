...
class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        try:
            ...
            if not state or not state.get("campaign_id"):
                return respond(self, 404, {"error": "no campaign exists yet"})

            client = google_ads_client.get_client()
            rows = google_ads_client.pull_performance(client, state["campaign_id"], since)
            written = upsert_daily_performance(rows)

            # Roll up totals (since campaign start)
            all_perf = get_performance_range("2026-05-01", datetime.now(tz).date().isoformat())
            total_spend = sum(r.get("cost_cents") or 0 for r in all_perf)
            total_conv = sum(float(r.get("conversions") or 0) for r in all_perf)
            total_rev = sum(r.get("conversion_value_cents") or 0 for r in all_perf)
            updated = upsert_campaign_state({
            ...
            log_action("sync_google_ads", {"since": since, "rows_written": written}, success=True)
